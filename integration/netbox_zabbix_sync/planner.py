...
def reconcile(
    ...
    for candidate in candidates:
        ...
        main_agent = next(
            (item for item in existing.get("interfaces", []) if item.get("type") == "1" and item.get("main") == "1"),
            None,
        )
        current_ip = main_agent.get("ip") if main_agent and main_agent.get("useip") == "1" else None
